...
@BindingAdapter("stateTrial3")
fun TextView.setStateTrialButtonTryForFree(stateTrial: StateTrial? = StateTrial.Loading) {
...
fun TextView.setStateInduction(
    stateTrial: StateTrial? = StateTrial.Eligible,
...
@BindingAdapter("stateForShield")
fun TextView.setTextForShield(stateTrial: StateTrial? = StateTrial.Eligible) {
    if (stateTrial == StateTrial.Eligible) {
        text = context.getString(R.string._3_day_free_trial_enabled)
    } else {
        text = context.getString(R.string.cancel_anytime)
    }
}
